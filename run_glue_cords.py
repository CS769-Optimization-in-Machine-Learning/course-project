...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="random",
        help="The strategy to use for coreset selection",
        choices=["glister", "craig", "gradmatch", "random", "submodular"],
    )
    parser.add_argument(
        "--log_file",
        type=str,
...
def main():
    ...
    LOSS_no_reduction=CrossEntropyLoss(reduction="none")

    # strategy=GLISTERStrategy(
    #     trainloader=train_dataloader,
    #     valloader=eval_dataloader,
    #     model=model,
    #     loss_func=LOSS_no_reduction,
    ...
    #     logger=logger
    # )
    # idx, gammas=strategy.select(budget=30000, model_params=model.state_dict())
    strategy=RandomStrategy(
        trainloader=train_dataloader,
    )
    idx, gammas=strategy.select(budget=30000)
    # strategy=CRAIGStrategy(
    #     trainloader=train_dataloader,
    #     valloader=eval_dataloader,
    #     model=model,
    #     loss=LOSS_no_reduction,
    #     device=accelerator.device,
    #     num_classes=2,
    #     linear_layer=True,
    #     if_convex=False,
    ...
    #     optimizer="lazy"
    # )
    # idx, gammas=strategy.select(budget=10000, model_params=model.state_dict())
    ...
    #     valid=True,
    # )
    # idx, gammas=strategy.select(budget=25000, model_params=model.state_dict())
    # strategy=SubmodularSelectionStrategy(
    #     trainloader=train_dataloader,
    #     valloader=eval_dataloader,
    #     model=model,
    #     loss=LOSS_no_reduction,
    ...
    train_data_subset=train_dataset.select(idx)
    train_dataloader=DataLoader(
        train_data_subset, shuffle=True, collate_fn=data_collator, batch_size=args.per_device_train_batch_size
    )
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            loss = LOSS(outputs, batch['labels'].view(-1))
        ...
        for step, batch in enumerate(eval_dataloader):
            outputs = model(**batch)
            predictions = outputs.argmax(dim=-1) if not is_regression else outputs.logits.squeeze()
            metric.add_batch(
